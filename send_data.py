...
def sending_stream(current_date, periods, num_records):
    ...
    try:
        ...
        if (date_dt >= datetime(year=2015, month=2, day=1)) and (
            date_dt <= datetime(year=2015, month=7, day=31)
        ):
            ...
            return print(
                "... Use format yyyy-mm-dd from 2015-02-01 up to 2015-07-31 ..."
            )
    except:
        return print(
            "... Use format yyyy-mm-dd from 2015-02-01 up to 2015-07-31 ..."
    ...
    with open('./targets/target.csv', 'w', encoding='utf-8') as f_in:
        for index in test_data:
            ...
            f_in.write(
                f"Record #{index} {record['id']}, {selling_price.iloc[index]}, {record['price_estimation']}\n"
            )
            sleep(0.1)
