...
class MonitoringService:
    ...
    def __init__(self, datasets: Dict[str, LoadedDataset], window_size: int):
        ...
        for dataset_info in datasets.values():
            ...
            self.monitoring[dataset_info.name] = ModelMonitoring(
                monitors=[
                    EVIDENTLY_MONITORS_MAPPING[k]()
                    for k in dataset_info.monitors
                ],
                options=[],
            )
    ...
    def iterate(self, dataset_name: str, new_rows: pd.DataFrame):
        ...
        if (
            ...
            and next_run_time > datetime.datetime.now()
        ):
            logging.info(
                "Next run for dataset %s at %s", dataset_name, next_run_time
            )
            return
