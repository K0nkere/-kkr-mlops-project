"""
Provides model trainig process
"""
...
def prepare_features(work_data, preprocessor=None):
    ...
    if not preprocessor:
        ...
        fm = FeaturesModifier(
            columns=num_2_impute + cat_2_impute + constant_2_impute + others
        )
        ...
        def objective(params):
            ...
            with mlflow.start_run():
                ...
                rmse_valid = mean_squared_error(
                    y_valid, prediction_valid, squared=False
                )
                ...
                print(
                    f'... Errors on valid: RMSE {rmse_valid} MAE {mae_valid} MAPE {mape_valid} ...',
                )
...
def train_best_models(
    ...
    for model in models:
        ...
        mlflow.set_experiment(
            best_models_experiment
        )  # "Auction-car-prices-best-models")
        ...
        with mlflow.start_run():
            ...
            best_params = json.loads(
                best_run[0].data.params['parameters'].replace("'", "\"")
            )
            staged_model = model(**best_params).fit(train, y_train)

            pipeline = Pipeline(
                steps=[('preprocessor', preprocessor), ('model', staged_model)]
            )
            predict_valid = pipeline.predict(X_valid)
            rmse_valid = mean_squared_error(
                y_valid, predict_valid, squared=False
            )
            ...
            print(
                "... {:} MODEL was saved as a RUN of {:} ...".format(
                    model.__name__, best_models_experiment
                )
            )
...
def model_to_registry(best_models_experiment, model_name, test_dataset_period):
    ...
    print(
        f"... Model RUN_ID {registered_model.run_id} was registered as version {registered_model.version} at {registered_model.current_stage} stage ..."
    )
...
@task
def model_promotion(
    current_date, model_name, registered_model_version, to_stage
):
    """
    ...
    print(
        f"... Model {model_name} version {registered_model_version} was promoted to {to_stage} {current_date} ..."
    )
...
def switch_model_of_production(X_test, y_test, model_name):
    ...
    if staging_model:
        ...
        mape_staging = mean_absolute_percentage_error(
            staging_test_prediction, y_test
        )
        print(
            f"... MAPE={mape_staging} for the model version {staging_version} ..."
        )
    else:
        mape_staging = np.inf

    production_model, production_version = load_model(
    ...
    if production_model:
        ...
        mape_production = mean_absolute_percentage_error(
            production_test_prediction, y_test
        )
        print(
            f"... MAPE={mape_production} for the model version {production_version} ..."
        )
    ...
    if mape_staging <= mape_production:
        print(
            f"... Need to switch models. Version {staging_version} is better than {production_version} ..."
        )
        ...
        print(
            f"... No need to switch models. Version {production_version} is the best ..."
        )
        return None
...
def main(current_date="2015-5-21", periods=5):
    ...
    best_models = params_search(
        train, valid, y_train, y_valid, train_dataset_period, models
    )
    ...
    registered_model = model_to_registry(
        best_models_experiment, model_name, test_dataset_period
    )
...
def retrain_request():
    ...
    check = {"service": "training_model"}

    response = requests.post(url=signal_url, json=check, timeout=60)
