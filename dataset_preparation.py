"""
Preparation of original dataset - split the data on monthly
"""
...
def main():
    ...
    data = pd.read_csv(
        './datasets/car_prices.csv',
        sep=',',
        error_bad_lines=False,
        parse_dates=['saledate'],
    )

    data['saledate'] = data['saledate'].apply(
        lambda t: datetime.strftime(t, "%Y-%m-%d %H:%M")
    )
    data['saledate'] = data['saledate'].apply(
    ...
    while period <= td_max:
        mask = (
            data['saledate'] < np.datetime64(period + relativedelta(months=1))
        ) & (data['saledate'] >= np.datetime64(period))

        filename = f"car-prices-{period.year}-{period.month}"
        print(filename, period + relativedelta(months=1), np.sum(mask))
